...
MILVUS_PORT = '19530'
DATA_BASE = 'migu_text'
MILVUS_COLLECTION_NAME = ('migu_text_collection')
...
def add_data():
    with open('question_parse_fail1.txt', 'r', encoding='utf-8') as rf:
        lines = rf.readlines()
        array = []
        for line in lines:
            try:
                ...
                for i in range(len(questions)):
                    ...
                    question_dict = {
                        ...
                        "embeddings": m.encode(question.get('question'), device='cuda').tolist(),
                        "status": 1
                    }
...
def add_question():
    ...
    for data in excel_datas:
        question_dict = {
            ...
            "answer": data[2],
            "embeddings": m.encode(data[1], device='cuda').tolist(),
            "status": 1
        }
        array.append(question_dict)
    insert_data(array)
...
def write_insert_fail_to_txt(question: str):
    filename = "question_insert_fail.txt"
...
def write_parse_fail_to_txt(question: str):
    filename = "question_parse_fail.txt"
    if os.path.exists(filename):
        # print("file exists")
        with open(filename, "a", encoding='UTF-8') as f:
            f.write(question)
